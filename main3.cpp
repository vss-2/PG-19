...
int aaa = 0;
	...
	vec3 PhongShading(vec3 normal, vec3 light_dir, float Ia, vec3 Ka, vec3 Kd, float Ks, int n, vec3 l, vec3 r, vec3 normCam)
	{
	...
	void pintarTriangulo(SDL_Renderer *renderer, const vec2 &v0, const vec2 &v1, const vec2 &v2, const Triangle &tr, const vec3 &light_dir)
	{
		...
		for(int x = xMin; x <= xMax; x++)
		{
			for (int y = yMin; y <= yMax; y++)
			{
				...
				{
					vec3 normTriag = (cross(tr.vertex[1].pos - tr.vertex[0].pos, tr.vertex[2].pos - tr.vertex[0].pos));
					vec3 Ka(210,210,210); 
					vec3 Kd(210,210,210); 
					float Ia = std::max(0.0f, dot(normTriag,(0,0,-1)));
					float Ks = 0.1; 
					int n = 50;
					vec3 l(0,0,-1);
					vec3 r( l - 2*(dot(normTriag,l)*normTriag));
					vec3 color = PhongShading(normTriag, light_dir, Ia, Ka, Kd, Ks, n, l, r, Pos-At);
